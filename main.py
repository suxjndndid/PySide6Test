...
class YoloPredictor(BasePredictor, QObject):
    ...
    def write_results(self, idx, results, batch):
        ...
        if len(det) == 0:
            return f'{log_string}(no detections), '  # if no detections, send this
        ...
        for c in det.cls.unique():
            ...
            class_counts[class_name] = int(n)
            log_string += f"{n}~{class_name},"  # Ensure correct format: "count~class_name,"

        # Write detections to file and draw on image
        ...
        self.labels_dict = class_counts

        # Debugging: Print the log_string to check its format
        print(f"Generated log_string: {log_string}")
